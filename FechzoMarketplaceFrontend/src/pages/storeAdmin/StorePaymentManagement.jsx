...
const StorePaymentManagement = () => {
  ...
  const [fromDate, setFromDate] = useState(startOfDay(new Date())); // Today by default
  const [toDate, setToDate] = useState(endOfDay(new Date()));       // Today by default
  ...
  const fetchPayments = useCallback(async () => {
    ...
    try {
      ...
      const storeId =
        storeProfile._id || storeProfile.storeId || localStorage.getItem("storeId");
  ...
  const applyQuickFilter = (type) => {
    ...
    switch (type) {
      ...
      case "week":
        ...
        break;
      case "month":
        from = startOfDay(subDays(today, 29)); // Last 30 days
        break;
      default:
        from = null;
        to = null;
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-6">
          ...
          <SummaryCard
            title="COD / Online"
            value={`${summary.codCount} / ${summary.onlineCount}`}
          />
        </div>
      )}
        ...
        >
          ...
          <div>
            ...
            <DatePicker
              ...
              onChange={(date) => {
                setFromDate(date);
                setActiveQuickFilter(""); // Clear quick filter when manual date selected
                setPagination((p) => ({ ...p, page: 1 }));
              }}
                ...
                payments.map((p) => (
                  <tr key={p._id || p.orderId} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm font-medium text-gray-900">
                      {p.orderId}
                      {p.paymentId && (
                        <div className="text-xs text-gray-400 mt-0.5 truncate max-w-[140px]" title={p.paymentId}>
                          {p.paymentId}
                        </div>
                      )}
                    </td>
              ...
              {getPageNumbers().map((pageNum, idx) =>
                pageNum === "..." ? (
                  ...
                  <button
                    ...
                    className={`min-w-9 px-3 py-1.5 text-sm border rounded-lg ${
